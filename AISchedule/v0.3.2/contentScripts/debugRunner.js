...
const runDebug = async ({ provider, parser, timer }, sendResponse) => {
  ...
  try {
    ...
    consoleRed('函数cheduleHtmlParser运行出错')
    console.error(err)
    return failBack('函数cheduleHtmlParser运行出错')
  }
  // 返回值类型检查
  if (!Array.isArray(parserRes)) {
    consoleRed('Parser返回值数据类型不为 Array')
    return failBack('Parser返回值数据类型不为 Array')
  }
  // 展示函数运行结果
  ...
  try {
    ...
    const validateRes = validateCourseInfos(parserRes)
